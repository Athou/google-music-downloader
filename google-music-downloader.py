...
if len(sys.argv) == 1:
  print "usage: google-music-downloader <login> <password> <target directory> <album id>"
  sys.exit()
...
dirName = "%s - %s" % (album["artist"], album["name"])
...
for song in album["tracks"]:
  ...
  fileName = "%s. %s - %s.mp3" % (song["trackNumber"], song["artist"], song["title"])
